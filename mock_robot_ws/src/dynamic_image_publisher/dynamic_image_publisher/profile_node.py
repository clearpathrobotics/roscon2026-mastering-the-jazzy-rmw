...
statistics (mean / p95 / peak). Intended to quantify the cost of
``dynamic_image_node`` but works for any node whose process command line contains the
given match string.

Run it inside the same container / PID namespace as the node, e.g.::

    ros2 run dynamic_image_publisher profile_node
    ros2 run dynamic_image_publisher profile_node --match dynamic_image_node --duration 15

The compositor rotates and alpha-blits the sprite onto the map every tick regardless of
motion, but a *moving* robot (with a changing yaw) enlarges the rotated sprite's
bounding box and shifts the dirty rectangle each frame, so it exercises the most
expensive compositing path. To contrast a stationary robot against a moving one, drive
the pose broadcaster while sampling::

    ros2 run dynamic_image_publisher profile_node --compare
"""
...
class _PoseMover:
    """Drive a ``pose_broadcaster`` node's pose parameters to move the robot frame.

    Uses an rclpy parameter client to set the ``x``/``y``/``yaw`` parameters of the
    target broadcaster node on each call, oscillating the robot pose so the
    ``dynamic_image_node`` re-composites a translated and re-rotated sprite every tick
    (the most expensive compositing path). rclpy is imported lazily so the profiler has
    no ROS dependency unless movement is actually requested.
    """
    ...
    def __call__(self, i: int) -> None:
        """Move the robot to a new pose on a circle for sample index ``i``."""
        ...
        future = self._client.set_parameters(
            [
                ...
                Parameter("yaw", Parameter.Type.DOUBLE, theta),
            ]
        )
...
def sample(
    ...
    (e.g. move the robot frame) so the measurement covers the moving-sprite path.
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--match",
        default="dynamic_image_node",
        help="Substring to match against process command lines (default: dynamic_image_node).",
    )
    parser.add_argument(
        "--duration",
        type=float,
    ...
    parser.add_argument(
        "--move",
        action="store_true",
        help="Move the robot while sampling (exercises the moving-sprite composite path).",
    )
    parser.add_argument(
        "--compare",
        action="store_true",
        help="Sample a stationary window then a moving window and report the delta.",
    )
    parser.add_argument(
        "--move-node",
        default="/dynamic_image_pose_broadcaster",
        help="Pose broadcaster node to drive when moving (default: "
        "/dynamic_image_pose_broadcaster).",
    )
    ...
    try:
        ...
        if args.compare:
            ...
            _print_report("stationary", static_stats, mem_label)
            moving_stats = sample(
                proc, args.duration, args.interval, args.uss, on_tick=mover
            )
            _print_report("moving", moving_stats, mem_label)
            delta = moving_stats["cpu_mean"] - static_stats["cpu_mean"]
            print(
                f"[delta] moving - stationary CPU mean = {delta:+.1f}%  "
                "(cost of re-compositing a translating, rotating sprite)"
            )
        else:
            stats = sample(
                proc,
            ...
            label = "moving" if args.move else "stationary"
